...
$node->safe_psql("postgres", q{
	CREATE FUNCTION c_ivfflat_should_visit_vacuum_list_page(integer) RETURNS boolean
	AS '$libdir/vector', 'vector_ivfflat_should_visit_vacuum_list_page'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION rust_ivfflat_should_visit_vacuum_list_page(integer) RETURNS boolean
...
$node->safe_psql("postgres", q{
	CREATE FUNCTION c_ivfflat_should_write_list_insert_page(integer, integer) RETURNS boolean
	AS '$libdir/vector', 'vector_ivfflat_should_write_list_insert_page'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION rust_ivfflat_should_write_list_insert_page(integer, integer) RETURNS boolean
...
$node->safe_psql("postgres", q{
	CREATE FUNCTION c_ivfflat_should_write_list_start_page(integer, integer) RETURNS boolean
	AS '$libdir/vector', 'vector_ivfflat_should_write_list_start_page'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION rust_ivfflat_should_write_list_start_page(integer, integer) RETURNS boolean
	AS '$libdir/vector', 'vector_rust_ivfflat_should_write_list_start_page'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION c_ivfflat_insert_page_is_valid(integer) RETURNS boolean
	AS '$libdir/vector', 'vector_ivfflat_insert_page_is_valid'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION rust_ivfflat_insert_page_is_valid(integer) RETURNS boolean
	AS '$libdir/vector', 'vector_rust_ivfflat_insert_page_is_valid'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION c_ivfflat_vacuum_page_is_valid(integer) RETURNS boolean
	AS '$libdir/vector', 'vector_ivfflat_vacuum_page_is_valid'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
$node->safe_psql("postgres", q{
	CREATE FUNCTION rust_ivfflat_vacuum_page_is_valid(integer) RETURNS boolean
	AS '$libdir/vector', 'vector_rust_ivfflat_vacuum_page_is_valid'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
});
...
my $visit_vacuum_list_page_parity = $node->safe_psql("postgres", q{
	SELECT c_ivfflat_should_visit_vacuum_list_page(page_no) =
		   rust_ivfflat_should_visit_vacuum_list_page(page_no)
	FROM (VALUES
		(-1),
		(0),
		(1),
...
my $write_list_insert_page_parity = $node->safe_psql("postgres", q{
	SELECT c_ivfflat_should_write_list_insert_page(insert_page, current_page) =
		   rust_ivfflat_should_write_list_insert_page(insert_page, current_page)
	FROM (VALUES
		(-1, -1),
		(5, -1),
		(5, 5),
...
my $write_list_start_page_parity = $node->safe_psql("postgres", q{
	SELECT c_ivfflat_should_write_list_start_page(start_page, current_page) =
		   rust_ivfflat_should_write_list_start_page(start_page, current_page)
	FROM (VALUES
		(-1, -1),
		(5, -1),
		(5, 5),
		(5, 9)
	) AS t(start_page, current_page);
});
is($write_list_start_page_parity, "t\nt\nt\nt");

my $insert_page_is_valid_parity = $node->safe_psql("postgres", q{
	SELECT c_ivfflat_insert_page_is_valid(page_no) =
		   rust_ivfflat_insert_page_is_valid(page_no)
	FROM (VALUES
		(-1),
		(0),
		(1),
...
is($insert_page_is_valid_parity, "t\nt\nt\nt");

my $vacuum_page_is_valid_parity = $node->safe_psql("postgres", q{
	SELECT c_ivfflat_vacuum_page_is_valid(page_no) =
		   rust_ivfflat_vacuum_page_is_valid(page_no)
	FROM (VALUES
		(-1),
		(0),
		(1),
...
is($vacuum_page_is_valid_parity, "t\nt\nt\nt");
